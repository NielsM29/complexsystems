...
df <- read.csv("wolf_sheep_grasspredation.csv", header = 1, skip = 22, col.names = c('', 'time', 'sheep', 'wolves', 'grass'))
df[,1] <- NULL
...
#Phase Space Reconstruction (Takens') & Simple plots
...
plot3d(df$sheep, df$wolves, df$grass, type = "l", col = gg.col(n = 5, alpha = 0.7))
